...
class CachedEpisodeDataset:
    def __init__(
        ...
        for image_path in iter_images(self.image_root, split=split, limit=limit):
            ...
            hard_current = hard_exists and hard_region_cache_is_current(hard_path, hard_region_metadata)
            if det_current and hard_current:
                ...
                elif not hard_current:
                    self.stale_hard_region += 1
        if not self.samples:
        ...
        if self.total_images != len(self.samples):
            print(
                ...
                f"missing_hard_region={self.missing_hard_region}, stale_hard_region={self.stale_hard_region})"
            )
