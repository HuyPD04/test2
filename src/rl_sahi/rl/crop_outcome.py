...
class CropOutcome:
    ...
    def info(self) -> dict[str, int | float | bool]:
        return {
            ...
            "crop_new_detection_max_score": float(self.new_detection_max_score),
...
class CropOutcomeEvaluator:
    ...
    def initial_new_count(
        ...
        _full_count, slice_count = _merged_source_counts(
            ...
            use_wbf=self.cfg.use_wbf,
            nms_type=self.cfg.nms_type,
        )
    ...
    def evaluate_from_predictions(
        ...
        new_detection_gain, new_detection_utility, new_detection_max_score = new_detection_stats_after_merge(
            det.image_shape,
            self.cfg.merge_iou,
        ...
        accepted = accepts_novel_detections(
            new_detection_gain,
            new_detection_utility,
            new_detection_max_score,
            self.cfg.min_slice_detections,
            self.cfg.min_slice_utility,
            self.cfg.min_new_detection_score,
        )
        return CropOutcome(
            boxes=boxes,
            scores=scores,
