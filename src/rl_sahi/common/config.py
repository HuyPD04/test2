...
class ProjectConfig:
    ...
    def target_classes(self) -> tuple[int, ...]:
        raw = self.section("classes").get("target_classes")
        if raw is None:
            raise ValueError("classes.target_classes must be configured explicitly")
        if isinstance(raw, str):
            values = tuple(int(x.strip()) for x in raw.split(",") if x.strip())
        else:
            values = tuple(int(x) for x in raw)
        if not values:
            raise ValueError("classes.target_classes must contain at least one class id")
        if len(set(values)) != len(values):
            raise ValueError("classes.target_classes must not contain duplicates")
        return values
