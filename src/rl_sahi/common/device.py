...
def _resolve_explicit_torch_device(value: str | torch.device) -> torch.device:
    requested = torch.device(value)
    if requested.type == "cuda" and not _cuda_available():
...
def resolve_torch_device(device: DeviceLike = None) -> torch.device:
    if isinstance(device, torch.device):
        return _resolve_explicit_torch_device(device)
    ...
    if device is not None:
        ...
        if normalized in {"", "auto"}:
            ...
            return _resolve_explicit_torch_device(value)
