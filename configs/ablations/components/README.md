...
The four `w/o` runs use the same base configuration, data, cached detector state, crop-outcome reward, action mask, and training budget. Train them from scratch with separate output directories:
...
Then benchmark the four `w/o` checkpoints on the same validation split:

```powershell
conda run -n doan python scripts/run_component_ablation.py --split val
```

`full.yaml` remains available for training and benchmarking the full RL-SAHI baseline separately.
