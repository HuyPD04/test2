...
class SliceEnvRewardTest(unittest.TestCase):
    ...
    def test_accepted_crop_without_hard_hit_keeps_positive_crop_reward(self) -> None:
        outcome = CropOutcome(
            boxes=np.zeros((0, 4), dtype=np.float32),
            scores=np.zeros((0,), dtype=np.float32),
            classes=np.zeros((0,), dtype=np.float32),
            new_detection_gain=1,
            new_detection_utility=0.6,
            new_detection_max_score=0.6,
        ...
        cfg = TrainConfig(
            crop_outcome_reward_scale=1.0,
            crop_attempt_penalty=0.25,
            hard_hit_reward=0.5,
        )
        reward = _terminal_reward_with_crop_outcome(0.0, outcome, 0, cfg)
